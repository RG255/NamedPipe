Function Send-Data
{
	...
	Try
	{
		...
		{
			Write-Host "DEBUG Send-Data: About to serialize (ChunkSize=$($PipeInfo.$StrChunkSize), Depth=$($PipeInfo.$StrDepth))" -ForegroundColor Yellow
		}
		...
		if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
		{
			Write-Host "DEBUG Send-Data: Serialized, type=$($serialized.GetType().Name), length=$($serialized.Length)" -ForegroundColor Yellow
		}
		...
		if ($serialized -is [array] -and $serialized[0].IsChunked)
		{
			...
			if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
			{
				Write-Host "DEBUG Send-Data: Chunked transfer, $totalChunks chunks" -ForegroundColor Yellow
			}
			...
			foreach ($chunk in $serialized)
			{
				...
				if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
				{
					Write-Host "DEBUG Send-Data: Sent chunk $chunkNum of $totalChunks" -ForegroundColor Cyan
				}
			}
		}
		else
		{
			...
			if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
			{
				Write-Host "DEBUG Send-Data: Single message, about to WriteLine" -ForegroundColor Yellow
			}
			$PipeInfo.$StrWriter.WriteLine($serialized)
			$PipeInfo.$StrWriter.Flush()
			if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
			{
				Write-Host "DEBUG Send-Data: Flush done" -ForegroundColor Green
			}
		}
		...
		If ($DataObject.$StrServerPID -ne $PID)
		{
			if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
			{
				Write-Host "DEBUG Send-Data: Client waiting for response..." -ForegroundColor Yellow
			}
		...
		else
		{
			if ($PipeInfo.$StrInfoDisplay -band $InfoDisplayBitDebug)
			{
				Write-Host "DEBUG Send-Data: Server send complete, returning" -ForegroundColor Green
			}
		}
	}
	catch
